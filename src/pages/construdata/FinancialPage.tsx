...
import { AreaChart, BarChart, PieChart, LineChart } from '../../components/charts/Charts'
...
export function FinancialPage() {
  ...
  const projectionData = useMemo(() => {
    ...
    const monthlyRecurring = prodTx
      .filter((t) => t.recurrence === 'monthly')
      .reduce((s, t) => s + (t.type === 'income' ? t.amount : -t.amount), 0)
    let cumulative = balance
    return Array.from({ length: months }, (_, i) => {
      cumulative += monthlyRecurring
      const label = projTab === 'anual'
        ? `${Math.floor(i / 12) + 2026} T${Math.floor((i % 12) / 3) + 1}`
        : MONTHS[i % 12]
      return { label, value: Math.max(0, cumulative) }
    })
  ...
  const installmentOptions = useMemo(() => {
    ...
    return tiers.map((t) => {
      const monthlyPayment = (income * t.pct) / 100
      const months = Math.ceil(price / monthlyPayment)
      const fits = monthlyPayment <= free
      return { ...t, monthlyPayment, months, fits, free }
    })
  ...
  function saveTx() {
    if (!txForm.description || !txForm.date) return
    if (editingTx) updateTransaction(editingTx.id, txForm, PRODUCT)
    else addTransaction(txForm, PRODUCT)
    ...
    <div className="space-y-5">
        ...
        <div className="flex bg-white rounded-xl border border-cyan-100 p-1 shadow-card">
          {TABS.map((t) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${mainTab === t.id ? 'bg-navy-800 text-white' : 'text-navy-500 hover:text-navy-800'}`}
            >
              {t.icon}{t.label}
            </button>
          ))}
        ...
        <div className="relative">
          ...
          {exportOpen && (
            <div className="absolute right-0 top-9 bg-white border border-navy-100 rounded-xl shadow-modal z-20 py-1 w-44">
              <button
                onClick={() => { exportTransactionsToPDF(prodTx, prodPayroll, 'ConstruData'); setExportOpen(false) }}
                className="flex items-center gap-2 w-full px-3 py-2 text-sm text-navy-700 hover:bg-surface"
              >
                <FilePDF size={14} className="text-red-500" /> Exportar PDF
              </button>
              <button
                onClick={() => { exportTransactionsToExcel(prodTx, prodPayroll, 'ConstruData'); setExportOpen(false) }}
                className="flex items-center gap-2 w-full px-3 py-2 text-sm text-navy-700 hover:bg-surface"
              >
      ...
      {mainTab === 'dashboard' && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <KPICard title="Receita Realizada" value={fmt(totalIncome)} trend={12} icon={<TrendingUp size={18} />} />
            <KPICard title="Despesas Pagas" value={fmt(totalExpense)} trend={-3} icon={<TrendingDown size={18} />} color="#EF4444" />
          ...
          {/* Receita vs Despesa */}
          <Card>
            <CardHeader title="Receita vs Despesa" subtitle="Últimos 12 meses (projeção)" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <Card>
              ...
              {incomeByCategory.length > 0 ? <PieChart data={incomeByCategory} /> : <p className="text-sm text-navy-400 py-8 text-center">Sem dados de receita</p>}
            </Card>
            <Card>
              <CardHeader title="Despesas por Categoria" />
              {expenseByCategory.length > 0 ? <PieChart data={expenseByCategory} /> : <p className="text-sm text-navy-400 py-8 text-center">Sem dados de despesa</p>}
            </Card>
          </div>
          ...
          <Card>
            <CardHeader
              ...
              action={<Button size="sm" onClick={() => setOkrModalOpen(true)}><Plus size={13} /> OKR</Button>}
            />
            {prodOKRs.length === 0 ? (
              <p className="text-sm text-navy-400 py-4 text-center">Nenhum OKR cadastrado.</p>
            ) : (
                        ...
                        return (
                          <div key={kr.id} className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                type="number"
                                className="w-20 text-xs border border-navy-100 rounded px-1.5 py-0.5 outline-none focus:border-cyan-400"
                                placeholder="Atual"
                                defaultValue={kr.current}
                                onBlur={(e) => updateKeyResult(okr.id, kr.id, { current: parseFloat(e.target.value) || 0 }, PRODUCT)}
                              />
                            </div>
                          </div>
                        )
          ...
          <Card>
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              <div>
                ...
                <p className="text-sm text-navy-400 mt-0.5">Baseado em transações recorrentes</p>
              </div>
              <div className="flex bg-surface rounded-lg p-0.5 border border-cyan-100">
                {(['mensal', 'semestral', 'anual'] as ProjTab[]).map((t) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-xs font-medium transition-colors capitalize ${projTab === t ? 'bg-navy-800 text-white' : 'text-navy-500 hover:text-navy-800'}`}
                  >
          ...
          {/* Payroll summary */}
          <Card>
            <CardHeader title="Folha de Pagamento" subtitle={`${prodPayroll[0]?.month ?? ''}`} />
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-navy-50">
                  ...
                  <th className="text-right py-2 text-xs font-semibold text-navy-500 uppercase">Salário Base</th>
                  <th className="text-right py-2 text-xs font-semibold text-navy-500 uppercase hidden lg:table-cell">Bônus</th>
                  <th className="text-right py-2 text-xs font-semibold text-navy-500 uppercase hidden lg:table-cell">Descontos</th>
              ...
              <tbody>
                {prodPayroll.map((e) => (
                  <tr key={e.id} className="border-b border-navy-50 hover:bg-surface/50">
                    ...
                    <td className="py-2.5 text-right text-green-600 hidden lg:table-cell">+{fmt(e.bonuses)}</td>
                    <td className="py-2.5 text-right text-red-500 hidden lg:table-cell">-{fmt(e.deductions)}</td>
                    <td className="py-2.5 text-right font-bold text-navy-800">{fmt(e.netSalary)}</td>
                    <td className="py-2.5">
                      <button
                        onClick={() => updatePayroll(e.id, { status: e.status === 'paid' ? 'pending' : 'paid' }, PRODUCT)}
                        className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border transition-colors ${e.status === 'paid' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}
                      >
                ...
                {filteredTx.map((t) => (
                  <tr key={t.id} className="border-b border-navy-50 hover:bg-surface/50 transition-colors">
                    <td className="px-4 py-3 text-navy-600 text-xs whitespace-nowrap">
                      ...
                      {t.dueDate && !t.paid && <div className="text-amber-600">Vence: {new Date(t.dueDate).toLocaleDateString('pt-BR')}</div>}
                    </td>
                    ...
                    <td className={`px-4 py-3 font-semibold text-right ${t.type === 'income' ? 'text-green-600' : 'text-red-500'}`}>
                      {t.type === 'income' ? '+' : '-'}{fmt(t.amount)}
                    </td>
                    <td className="px-4 py-3">
                      <button
                        onClick={() => updateTransaction(t.id, { paid: !t.paid }, PRODUCT)}
                        className={`flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border transition-colors ${t.paid ? 'bg-green-50 text-green-700 border-green-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}
                      >
      ...
      {mainTab === 'calculadora' && (
        <div className="space-y-6">
          {/* Seção A: Budget calculator */}
          <Card>
            <CardHeader title="Calculadora de Orçamento" subtitle="Entenda como sua renda se distribui" />
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
              <Input label="Renda mensal (R$)" type="number" value={calcIncome} onChange={(e) => setCalcIncome(e.target.value)} placeholder="Ex: 10000" />
              <Input label="Despesas fixas mensais (R$)" type="number" value={calcExpenses} onChange={(e) => setCalcExpenses(e.target.value)} placeholder="Ex: 4000" />
              <Input label="Meta de poupança (R$)" type="number" value={calcSavings} onChange={(e) => setCalcSavings(e.target.value)} placeholder="Ex: 1500" />
            </div>
            {calcIncome && (
              <>
                ...
                <div className="border-t border-navy-50 pt-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setCalcItems([...calcItems, { desc: '', amount: '' }])}>
          ...
          {/* Seção B: Installment calculator */}
          <Card>
            <CardHeader title="Parcela Saudável" subtitle="Descubra a melhor forma de parcelar sua compra" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
              <Input label="O que deseja comprar?" value={installItem} onChange={(e) => setInstallItem(e.target.value)} placeholder="Ex: Notebook, TV, Carro..." />
              <Input label="Preço total (R$)" type="number" value={installPrice} onChange={(e) => setInstallPrice(e.target.value)} placeholder="Ex: 5000" />
              <Input label="Renda mensal (R$)" type="number" value={installIncome} onChange={(e) => setInstallIncome(e.target.value)} placeholder="Ex: 10000" />
              <Input label="Despesas fixas mensais (R$)" type="number" value={installExpenses} onChange={(e) => setInstallExpenses(e.target.value)} placeholder="Ex: 4000" />
            </div>

            {installmentOptions.length > 0 && (
              <div className="space-y-3">
                {installItem && <p className="text-sm font-semibold text-navy-700">Opções de parcelamento para: <span className="text-cyan-600">{installItem}</span> — {fmt(parseFloat(installPrice))}</p>}
                <div className="flex items-center gap-2 text-sm text-navy-500 mb-2">
                  <Wallet size={14} className="text-cyan-500" />
                  Sobra livre após despesas: <span className="font-bold text-navy-800">{fmt((parseFloat(installIncome) || 0) - (parseFloat(installExpenses) || 0))}</span>
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-navy-50 bg-surface">
                        ...
                        <th className="text-right px-3 py-2 text-xs font-semibold text-navy-500 uppercase">% da Renda</th>
                        <th className="text-right px-3 py-2 text-xs font-semibold text-navy-500 uppercase">Valor/mês</th>
                        <th className="text-right px-3 py-2 text-xs font-semibold text-navy-500 uppercase">Nº Parcelas</th>
                        ...
                        <tr key={opt.pct} className={`border-b border-navy-50 ${!opt.fits ? 'opacity-50' : 'hover:bg-surface/50'}`}>
                          ...
                          <td className="px-3 py-2.5">
                            <Button
                              ...
                              variant="ghost"
                              onClick={() => addGoal({
                                title: `Poupar para: ${installItem || 'Compra'}`,
                                description: `Parcela ${opt.label}: ${fmt(opt.monthlyPayment)}/mês por ${opt.months} meses`,
                                target: parseFloat(installPrice) || 0,
                                current: 0,
                                unit: 'R$',
                                dueDate: new Date(Date.now() + opt.months * 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10),
                                status: 'on_track',
                              }, PRODUCT)}
                              className="text-cyan-600 hover:bg-cyan-50 whitespace-nowrap text-xs"
                            >
                              + Meta
                            </Button>
                          </td>
                ...
                {installmentOptions.some((o) => !o.fits) && (
                  <p className="text-xs text-amber-600">* Opções acinzentadas excedem sua sobra livre mensal ({fmt((parseFloat(installIncome) || 0) - (parseFloat(installExpenses) || 0))}).</p>
                )}
              </div>
            )}
          </Card>
      ...
      <Modal open={txModalOpen} onClose={() => setTxModalOpen(false)} title={editingTx ? 'Editar Lançamento' : 'Novo Lançamento'} size="lg">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input label="Data de vencimento (opcional)" type="date" value={txForm.dueDate ?? ''} onChange={(e) => setTxForm({ ...txForm, dueDate: e.target.value || undefined })} />
          </div>
          <Textarea label="Notas (opcional)" value={txForm.notes ?? ''} rows={2} onChange={(e) => setTxForm({ ...txForm, notes: e.target.value })} />
      ...
      <Modal open={okrModalOpen} onClose={() => setOkrModalOpen(false)} title="Novo OKR Financeiro">
        <div className="space-y-3">
          <Input label="Objetivo" value={okrForm.objective} onChange={(e) => setOkrForm({ ...okrForm, objective: e.target.value })} placeholder="Ex: Aumentar receita em 20% no Q2" />
          <div className="grid grid-cols-2 gap-3">
            <Input label="Período" value={okrForm.period} onChange={(e) => setOkrForm({ ...okrForm, period: e.target.value })} placeholder="Ex: Q2 2026" />
          ...
          <div className="flex gap-2 pt-2">
            <Button onClick={() => {
              ...
              addOKR({ objective: okrForm.objective, period: okrForm.period, status: okrForm.status, keyResults: [] }, PRODUCT)
              setOkrModalOpen(false)
              setOkrForm({ objective: '', period: '', status: 'on_track' })
            }}>Criar OKR</Button>
