...
const emptyForm = { description: '', type: 'income' as TransactionType, amount: '', category: '', date: '', dueDate: '', notes: '', paid: false }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <div className="flex items-center gap-2 mb-1"><TrendingUp size={16} className="text-green-500" /><p className="text-xs text-navy-400 font-medium">Receita Realizada</p></div>
          <p className="text-xl font-bold text-green-600">{fmt(totalIncome)}</p>
        </Card>
        <Card>
          <div className="flex items-center gap-2 mb-1"><TrendingDown size={16} className="text-red-500" /><p className="text-xs text-navy-400 font-medium">Despesas Pagas</p></div>
          <p className="text-xl font-bold text-red-500">{fmt(totalExpense)}</p>
        </Card>
        <Card>
          <div className="flex items-center gap-2 mb-1"><DollarSign size={16} className="text-cyan-500" /><p className="text-xs text-navy-400 font-medium">Saldo</p></div>
          <p className={`text-xl font-bold ${balance >= 0 ? 'text-green-600' : 'text-red-500'}`}>{fmt(balance)}</p>
        </Card>
        <Card>
          <div className="flex items-center gap-2 mb-1"><AlertCircle size={16} className="text-amber-500" /><p className="text-xs text-navy-400 font-medium">A Vencer</p></div>
          <p className="text-xl font-bold text-amber-600">{fmt(pending)}</p>
        </Card>
      </div>
        ...
        <div className="flex bg-white rounded-xl border border-cyan-100 p-1 shadow-card">
          {(['transactions', 'payroll'] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${tab === t ? 'bg-navy-800 text-white' : 'text-navy-500 hover:text-navy-800'}`}>
              {t === 'transactions' ? 'Lançamentos' : 'Folha de Pagamento'}
            </button>
          ))}
        </div>
        {tab === 'transactions' && (
          <Button size="sm" onClick={() => setAdding(true)}><Plus size={14} /> Lançamento</Button>
        )}
      </div>
              ...
              {prodTx.map((t) => (
                <tr key={t.id} className="border-b border-navy-50 hover:bg-surface/50 transition-colors">
                  <td className="px-4 py-3">
                    <p className="font-medium text-navy-800">{t.description}</p>
                    {t.dueDate && !t.paid && <p className="text-xs text-amber-600">Vence: {new Date(t.dueDate).toLocaleDateString('pt-BR')}</p>}
                  </td>
                  <td className="px-4 py-3 text-navy-500 hidden md:table-cell">{t.category}</td>
                  <td className="px-4 py-3">
                    <Badge variant={t.type === 'income' ? 'success' : 'danger'}>{t.type === 'income' ? 'Receita' : 'Despesa'}</Badge>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      className={`flex items-center gap-1 text-xs px-2 py-1 rounded-full border transition-colors ${t.paid ? 'bg-green-50 text-green-700 border-green-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}
                    >
                      {t.paid ? <><Check size={11} /> Pago</> : 'Pendente'}
                    </button>
                  </td>
                  <td className="px-4 py-3">
                    <button onClick={() => deleteTransaction(t.id, PRODUCT)} className="p-1.5 rounded text-navy-300 hover:text-red-500 hover:bg-red-50">×</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-navy-50 bg-surface">
                ...
                <th className="text-right px-4 py-3 text-xs font-semibold text-navy-500 uppercase hidden lg:table-cell">Bônus</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-navy-500 uppercase hidden lg:table-cell">Descontos</th>
            ...
            <tbody>
              {prodPayroll.map((e) => (
                <tr key={e.id} className="border-b border-navy-50 hover:bg-surface/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right text-green-600 hidden lg:table-cell">+{fmt(e.bonuses)}</td>
                  <td className="px-4 py-3 text-right text-red-500 hidden lg:table-cell">-{fmt(e.deductions)}</td>
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      className={`flex items-center gap-1 text-xs px-2 py-1 rounded-full border transition-colors ${e.status === 'paid' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}
                    >
      ...
      <Modal open={adding} onClose={() => setAdding(false)} title="Novo Lançamento">
        <div className="space-y-3">
          ...
          <Input label="Data de vencimento (opcional)" type="date" value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} />
          ...
          <div className="flex gap-2 pt-2">
            <Button onClick={() => {
              ...
              addTransaction({ description: form.description, type: form.type, amount: Number(form.amount), category: form.category, date: form.date, dueDate: form.dueDate || undefined, paid: form.paid, notes: form.notes }, PRODUCT)
