...
import { Plus, Search, Users, TrendingUp, Trash2, Edit2 } from 'lucide-react'
...
const statusBadge: Record<ClientStatus, 'success' | 'info' | 'default' | 'warning'> = {
  active: 'success', partner: 'info', prospect: 'warning', inactive: 'default'
}
const statusLabel: Record<ClientStatus, string> = {
  active: 'Ativo', partner: 'Parceiro', prospect: 'Prospecto', inactive: 'Inativo'
}
const stageLabel: Record<DealStage, string> = {
  lead: 'Lead', qualified: 'Qualificado', proposal: 'Proposta', negotiation: 'Negociação', won: 'Ganho', lost: 'Perdido'
}
const stageBadge: Record<DealStage, 'default' | 'info' | 'warning' | 'success' | 'danger'> = {
  lead: 'default', qualified: 'info', proposal: 'warning', negotiation: 'navy' as 'default', won: 'success', lost: 'danger'
}
...
export function CRMPage() {
  ...
  const filteredClients = prodClients.filter((c) =>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card><p className="text-xs text-navy-400 font-medium">Negócios Ganhos</p><p className="text-2xl font-bold text-navy-800 mt-1">{wonDeals}</p></Card>
      </div>
          ...
          {prodDeals.map((deal) => (
            <Card key={deal.id}>
              ...
              <div className="flex items-center gap-4 mt-3">
                <div>
                  <p className="text-xs text-navy-400">Valor</p>
                  <p className="font-bold text-navy-800">R$ {deal.value.toLocaleString('pt-BR')}</p>
                </div>
                <div>
                  <p className="text-xs text-navy-400">Probabilidade</p>
                  <p className="font-bold text-navy-800">{deal.probability}%</p>
                </div>
                <div>
                  <p className="text-xs text-navy-400">Fechamento</p>
                  <p className="font-medium text-navy-700 text-sm">{new Date(deal.expectedClose).toLocaleDateString('pt-BR')}</p>
                </div>
              </div>
              <div className="mt-3">
      ...
      <Modal open={addingDeal} onClose={() => setAddingDeal(false)} title="Novo Negócio">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input label="Fechamento previsto" type="date" value={dealForm.expectedClose} onChange={(e) => setDealForm({ ...dealForm, expectedClose: e.target.value })} />
          </div>
          <div className="flex gap-2 pt-2">
            <Button onClick={() => {
