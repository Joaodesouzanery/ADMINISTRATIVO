      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-sm border border-navy-100 rounded-lg bg-white outline-none focus:border-cyan-400 text-navy-800 placeholder-navy-400"
          />
        </div>
        <select
          value={filterType}
          onChange={(e) => setFilterType(e.target.value as DocType | 'all')}
          className="border border-navy-100 rounded-lg px-3 py-2 text-sm bg-white outline-none text-navy-700"
        >
          ...
          return (
            <Card key={doc.id} className="hover:shadow-md transition-shadow">
              ...
              <div className="flex gap-2 mt-3">
                ...
                <Button variant="ghost" size="sm" onClick={() => deleteDocument(doc.id, PRODUCT)} className="text-red-500 hover:bg-red-50">
                  <Trash2 size={13} />
                </Button>
              </div>
            </Card>
      ...
      <Modal open={adding} onClose={() => setAdding(false)} title="Novo Documento" size="lg">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input label="Tags (separar por vírgula)" value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} />
          </div>
          <Textarea label="Conteúdo / Descrição" value={form.content} rows={5} onChange={(e) => setForm({ ...form, content: e.target.value })} placeholder={form.type === 'prompt' ? 'Digite o prompt de IA aqui...' : 'Descrição ou conteúdo do documento...'} />
          <div className="flex gap-2 pt-2">
            <Button onClick={() => {
              ...
              setAdding(false)
              setForm({ title: '', type: 'document', category: '', content: '', version: '1.0', tags: '' })
            }}>Salvar</Button>
          ...
          <div className="space-y-3">
            ...
            {viewing.content && (
              <div className="bg-surface rounded-lg p-4">
                <pre className="text-sm text-navy-700 whitespace-pre-wrap font-sans">{viewing.content}</pre>
              </div>
            )}
            <p className="text-xs text-navy-400">Criado: {new Date(viewing.createdAt).toLocaleDateString('pt-BR')} · Atualizado: {new Date(viewing.updatedAt).toLocaleDateString('pt-BR')}</p>
          </div>
        )}
      </Modal>
    </div>
