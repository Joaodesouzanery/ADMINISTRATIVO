      ...
      {tab === 'goals' && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <Button onClick={() => setAddingGoal(true)} size="sm"><Plus size={14} /> Nova Meta</Button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {prodGoals.map((goal: Goal) => {
              ...
              return (
                <Card key={goal.id}>
                  ...
                  <div className="h-2 bg-navy-100 rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, backgroundColor: goal.status === 'completed' ? '#22C55E' : goal.status === 'at_risk' ? '#F59E0B' : '#7DD4A8' }} />
                  </div>
      ...
      <Modal open={addingGoal} onClose={() => setAddingGoal(false)} title="Nova Meta">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input label="Unidade" value={goalForm.unit} onChange={(e) => setGoalForm({ ...goalForm, unit: e.target.value })} />
          </div>
